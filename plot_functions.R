...
#-----------------------------------------------------------
map_setup <- function(df, min_lat=NULL, max_lat=NULL, min_lon=NULL, max_lon=NULL,
  ...
  myLocation <- c(min_lon+minLonAdj, min_lat+minLatAdj,
                  max_lon+maxLonAdj, max_lat+maxLatAdj)
  #myLocation <- c(minLon+4, minLat+3, maxLon-3, maxLat-6)
  bg_map <- ggmap::get_map(location=myLocation, source="stamen", maptype="terrain", crop=TRUE)
...
plot_map_errors <- function(bg_map, stats_df, xcoln="lon", ycoln="lat",
  ...
  gg_err <- ggmap::ggmap(bg_map) +
    ...
    ggplot2::scale_size(size_label, range=c(size_min_pt, size_max_pt),
    ...
    ggplot2::theme(plot.title=element_text(size=18,face="bold", vjust=-1, hjust=0.5)) +
  ...
  gg_hist <- ggplot(data=subset(my_data, !is.na(val_coln)),
    ...
    ggplot2::geom_bar() +
    ggplot2::labs(x=color_label, y="Site Count") +
    ggplot2::ggtitle(bquote(atop(.(paste0("Distribution of ", color_label)),
